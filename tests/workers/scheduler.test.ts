...
describe('InMemoryWorkerJobRepository', () => {
  ...
  it('falls through when idempotency index points at a missing job', async () => {
    const repository = new InMemoryWorkerJobRepository();
    const queuedAt = baseDate.toISOString();
    ...
    const jobs = Reflect.get(repository, 'jobs') as Map<string, unknown>;
    ...
    await expect(repository.getById('job-1')).resolves.toBeNull();
    await expect(repository.getById('job-2')).resolves.toMatchObject({
      id: 'job-2',
      idempotencyKey: 'stale-key',
    });
    await expect(repository.list()).resolves.toHaveLength(1);
...
describe('worker scheduler helpers', () => {
  ...
  it('uses the default now provider when none is passed', async () => {
    const repository = new InMemoryWorkerJobRepository();
    const connectorFactory = new FakeConnectorFactory([]);
    const secrets = makeSecrets('credential-default-now');
    ...
    await scheduler.enqueueProviderSync({
      provider: 'openai',
      accountId: 'credential-default-now',
      startTime: 100,
      endTime: 200,
    });
    ...
    expect(connectorFactory.calls).toHaveLength(1);
  });

  it('fails unsupported job types through assertNever', async () => {
    const repository = new InMemoryWorkerJobRepository();
    const connectorFactory = new FakeConnectorFactory([]);
    const secrets = makeSecrets('credential-1');
...
function makeSecrets(accountId: string): SecureSecretService {
  const secretsRepository = new InMemorySecureStorageRepository();
  const secrets = new SecureSecretService(
    secretsRepository,
    new StaticKeyProvider(Buffer.alloc(32, 9)),
